...
function isRegistrationClosed() {
  return ['closed', 'off', 'disabled', 'private'].includes(REGISTRATION_MODE);
}
...
async function enforceRegistrationLimits(req, email) {
  ...
  if ((recent?.count || 0) >= maxAttempts) {
    await dbRun(
      'INSERT INTO registration_attempts (ip_hash, email_domain, result) VALUES (?, ?, ?)',
      [ipHash, emailDomain, 'rate_limited']
    );
...
function setupDatabase() {
  ...
  db.serialize(() => {
    ...
    db.run(`CREATE INDEX IF NOT EXISTS idx_registration_attempts_ip_created ON registration_attempts (ip_hash, created_at)`);
...
app.post('/api/register', async (req, res) => {
  ...
  if (username.length < 3 || username.length > 32) {
    return res.status(400).json({ error: 'Login must be 3-32 characters' });
  }
